...
def handle_bar(context, bar_dict):
    ...
    # 策略逻辑
    if price1['bid'][0] - price2['ask'][0] > 5:
        if context.orderFlag1:
            order1 = order_lots(context.s1, 1, style=LimitOrder(price1['bid'][0] * 0.99))
            order2 = order_lots(context.s2, 1, style=LimitOrder(price2['ask'][0] * 1.01))
            all_text = ''
    ...
    if price1['ask'][0] - price2['bid'][0] < 3.5:
        if context.orderFlag2:
            order1 = order_lots(context.s1, 1, style=LimitOrder(price1['ask'][0] * 1.01))
            order2 = order_lots(context.s2, -1, style=LimitOrder(price2['bid'][0] * 0.99) )
            if order1.status == ORDER_STATUS.FILLED and order2.status == ORDER_STATUS.FILLED:
                info1 = insert_2_text(context.logFileName, order1)  # order信息 保存到本地
                info2 = insert_2_text(context.logFileName, order2)  # order信息 保存到本地
